"""
Chat Simple tool - General development chat and collaborative thinking (SimpleTool architecture)

This is a parallel implementation of the Chat tool using the new SimpleTool architecture.
It maintains identical behavior and schema compatibility with the original Chat tool
while leveraging the enhanced SimpleTool base class for cleaner architecture.

This tool serves as the migration target from tools/chat.py (old architecture)
to tools/simple/chat_simple.py (new architecture).
"""
...
class ChatSimpleRequest(ToolRequest):
    """Request model for ChatSimple tool - matches original ChatRequest exactly"""
...
class ChatSimpleTool(SimpleTool):
    """
    ...
    def _get_available_models(self) -> list[str]:
        """
        Override to ensure ChatSimple gets the same models as the original Chat tool.

        This temporary workaround addresses the issue where the tool name "chat_simple"
        might be treated differently by model restrictions compared to "chat".
        """
        # Temporarily create a Chat tool instance to get its available models
        from tools.chat import ChatTool

        temp_chat = ChatTool()
        return temp_chat._get_available_models()

    def get_request_model(self):
        """Return the ChatSimple-specific request model"""
        return ChatSimpleRequest
    ...
    async def prepare_prompt(self, request: ChatSimpleRequest) -> str:
        """
    ...
    def format_response(self, response: str, request: ChatSimpleRequest, model_info: Optional[dict] = None) -> str:
        """
