"""
Planner Workflow tool - Sequential planning with workflow architecture

This tool provides the same planning capabilities as the original planner tool
but uses the new workflow architecture for consistency with other tools like debug.

The tool guides users through step-by-step planning with forced pauses between
steps to ensure thorough consideration of alternatives, dependencies, and
strategic decisions before moving to tactical details.

Key differences from original planner:
- Uses workflow architecture (inherits from WorkflowTool)
- Implements proper workflow patterns with BaseWorkflowMixin
- Self-contained (no expert analysis required)
- Maintains all original planner functionality
"""
...
PLANNER_WORKFLOW_FIELD_DESCRIPTIONS = {
    "step": (
...
class PlannerWorkflowRequest(WorkflowRequest):
    ...
    step: str = Field(..., description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["step"])
    step_number: int = Field(..., description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["step_number"])
    total_steps: int = Field(..., description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["total_steps"])
    next_step_required: bool = Field(..., description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["next_step_required"])

    # Optional revision/branching fields (planning-specific)
    is_step_revision: Optional[bool] = Field(False, description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["is_step_revision"])
    revises_step_number: Optional[int] = Field(None, description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["revises_step_number"])
    is_branch_point: Optional[bool] = Field(False, description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["is_branch_point"])
    branch_from_step: Optional[int] = Field(None, description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["branch_from_step"])
    branch_id: Optional[str] = Field(None, description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["branch_id"])
    more_steps_needed: Optional[bool] = Field(False, description=PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["more_steps_needed"])

    # Exclude all investigation/analysis fields that aren't relevant to planning
    findings: str = Field(default="", exclude=True, description="Not used for planning - step content serves as findings")
    ...
    relevant_context: list[str] = Field(default_factory=list, exclude=True, description="Planning doesn't track code context")
...
class PlannerWorkflowTool(WorkflowTool):
    """
    ...
    def get_tool_fields(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "is_step_revision": {
                "type": "boolean",
                "description": PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["is_step_revision"],
            },
            "revises_step_number": {
                "type": "integer",
                "minimum": 1,
                "description": PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["revises_step_number"],
            },
            "is_branch_point": {
                "type": "boolean",
                "description": PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["is_branch_point"],
            },
            "branch_from_step": {
                "type": "integer",
                "minimum": 1,
                "description": PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["branch_from_step"],
            },
            "branch_id": {
                "type": "string",
                "description": PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["branch_id"],
            },
            "more_steps_needed": {
                "type": "boolean",
                "description": PLANNER_WORKFLOW_FIELD_DESCRIPTIONS["more_steps_needed"],
            },
        }
    ...
    def handle_work_continuation(self, response_data: dict, request) -> dict:
        ...
        required_actions = self.get_required_actions(
            request.step_number, "planning", request.step, request.total_steps
        )
