...
class OpenAIModelProvider(RegistryBackedProviderMixin, OpenAICompatibleProvider):
    ...
    def _validate_responses_api_config(self):
        """Validate Responses API configuration at startup.

        Checks if models configured with use_openai_response_api are known
        to support it, and warns about potential issues.
        """
        self._ensure_registry()
        ...
        # Show helpful info if there are any issues
        if warnings:
            logger.info(f"✅ Responses API: {_RESPONSES_API_EXAMPLES}")
            logger.info("❌ Chat only: gpt-3.5-turbo, gpt-4, gpt-4-turbo")
