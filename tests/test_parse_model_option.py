...
class TestParseModelOption:
    ...
    def test_openrouter_free_suffix_preserved(self):
        ...
        model, option = self.provider_manager.parse_model_option("openai/gpt-3.5-turbo:free")
    ...
    def test_openrouter_beta_suffix_preserved(self):
        ...
        model, option = self.provider_manager.parse_model_option("anthropic/claude-3-opus:beta")
    ...
    def test_openrouter_preview_suffix_preserved(self):
        ...
        model, option = self.provider_manager.parse_model_option("google/gemini-pro:preview")
    ...
    def test_ollama_tag_parsed_as_option(self):
        ...
        model, option = self.provider_manager.parse_model_option("llama3.2:latest")
        assert model == "llama3.2"
        assert option == "latest"

    def test_consensus_stance_parsed_as_option(self):
        """Test that consensus stances are parsed as options."""
        model, option = self.provider_manager.parse_model_option("o3:for")
        assert model == "o3"
        assert option == "for"

        model, option = self.provider_manager.parse_model_option("gemini-2.5-pro:against")
    ...
    def test_openrouter_unknown_suffix_parsed_as_option(self):
        ...
        model, option = self.provider_manager.parse_model_option("openai/gpt-4:custom-tag")
        assert model == "openai/gpt-4"
        assert option == "custom-tag"

    def test_plain_model_name(self):
        """Test plain model names without colons."""
        model, option = self.provider_manager.parse_model_option("gpt-4")
        assert model == "gpt-4"
        assert option is None

    def test_url_not_parsed(self):
        """Test that URLs are not parsed for options."""
        model, option = self.provider_manager.parse_model_option("http://localhost:8080")
