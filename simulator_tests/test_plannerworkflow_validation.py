...
Tests the plannerworkflow tool's capabilities using the new workflow architecture.
...
class PlannerWorkflowValidationTest(ConversationBaseTest):
    """Test plannerworkflow tool with new workflow architecture"""

    @property
    def test_name(self) -> str:
        return "plannerworkflow_validation"
    ...
    def run_test(self) -> bool:
        """Test plannerworkflow tool capabilities"""
        ...
        try:
            ...
            self.logger.info("  ✅ All plannerworkflow validation tests passed")
            return True
    ...
    def _test_single_planning_session(self) -> bool:
        ...
        try:
            ...
            response1_data = self._parse_plannerworkflow_response(response1)
            if not response1_data:
                return False

            # Validate step 1 response structure - expect pause_for_plannerworkflow for next_step_required=True
            if not self._validate_step_response(response1_data, 1, 4, True, "pause_for_plannerworkflow"):
                return False
            ...
            response2_data = self._parse_plannerworkflow_response(response2)
            if not self._validate_step_response(response2_data, 2, 4, True, "pause_for_plannerworkflow"):
                return False
            ...
            response3_data = self._parse_plannerworkflow_response(response3)
            if not response3_data:
                return False

            # Validate final response structure - should be self-contained completion
            if response3_data.get("status") != "plannerworkflow_complete":
                self.logger.error(f"Expected status 'plannerworkflow_complete', got '{response3_data.get('status')}'")
                return False
    ...
    def _test_planning_with_continuation(self) -> bool:
        ...
        try:
            ...
            if not self._validate_step_response(response1_data, 2, 2, True, "pause_for_plannerworkflow"):
                return False
    ...
    def _test_complex_plan_deep_thinking(self) -> bool:
        ...
        try:
            ...
            response4_data = self._parse_plannerworkflow_response(response4)
            if not response4_data:
                return False

            # Step 4 should use normal flow (no more deep thinking pauses)
            if response4_data.get("status") != "pause_for_plannerworkflow":
    ...
    def _test_self_contained_completion(self) -> bool:
        ...
        try:
            ...
            if response2_data.get("status") != "plannerworkflow_complete":
    ...
    def call_mcp_tool(self, tool_name: str, params: dict) -> tuple[Optional[str], Optional[str]]:
        """Call an MCP tool in-process - override for plannerworkflow-specific response handling"""
        ...
        # Extract continuation_id from plannerworkflow response specifically
        continuation_id = self._extract_plannerworkflow_continuation_id(response_text)

        return response_text, continuation_id

    def _extract_plannerworkflow_continuation_id(self, response_text: str) -> Optional[str]:
        """Extract continuation_id from plannerworkflow response"""
        try:
            # Parse the response
            ...
            self.logger.debug(f"Failed to parse response for plannerworkflow continuation_id: {e}")
            return None

    def _parse_plannerworkflow_response(self, response_text: str) -> dict:
        """Parse plannerworkflow tool JSON response"""
        try:
            # Parse the response - it should be direct JSON
            return json.loads(response_text)

        except json.JSONDecodeError as e:
            self.logger.error(f"Failed to parse plannerworkflow response as JSON: {e}")
    ...
    def _validate_step_response(
        ...
        """Validate a plannerworkflow step response structure"""
        try:
            # Check status
